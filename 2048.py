...
class Game2048(object):
    ...
    def __init__(self, size=None, grid=None, random_state=None):
        # misc
        if size is None:
            while not size in self._SIZES:
                try:
                    size = int(input_grabber(
        ...
        if grid is None:
            ...
            indices = list(np.ndindex((size, size)))
    ...
    def play(self):
        """Play game."""
        print("=" * 80)
        print("2048 : play the 2048 game")
        print(("The player enters direction for movement : I = up, J = left,"
               " K = down, L = right"))
